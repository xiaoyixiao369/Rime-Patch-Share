...
setlocal
:: ------------------------------------------------------------------------
:: 本脚本只适合Windows单设备使用，多设备同步不要使用词脚本。
:: 使用本脚本请将文件名改为：updateRepoAndSync.template.bat并重新部署一次，使用新生
:: 成的updateRepoAndSync.template.bat文件
:: ------------------------------------------------------------------------
:: 注意：
...
:: 功能：
:: 1. 同步当前设备用户词库
:: 2. 备份当前设备用户词库文件（设备词库目录下的userDictBakDir目录） 
:: 3. 清理当前设备用户词库 
:: 4. 清理历史备份的用户词库文件 
:: 5. 同步远程代码仓库
:: 6. 重新部署，同步用户词库
...
:: 2. 如果被删除的词在执行本脚本后仍然存在，请检查其它设备文件夹中是否有相同的词
...
echo 优先同步用户词库，将rime_ice.userdb文件夹中的修改同步到用户词库文件...
